...
// ================= CACHE CONFIGURATION (Superadmin Session) =================
...
const CustomerManagement = () => {
  ...
  // ================= DATA FETCHING (With Superadmin Cache) =================
  ...
  const handleCreateCustomer = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setToastMessage("Member Authorized & Registered 🎉");
        setShowToast(true);
        setForm({ name: "", username: "", password: "", phone: "" });
        setSelectedSchemeId(null);
        setShowForm(false);
        await fetchData(true); // Sync Superadmin Cache
      }
    } finally {
      setIsCreating(false);
    }
  ...
  const handleSaveNewPassword = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setToastMessage("Communication failure"); 
        setShowToast(true); 
    } finally {
  ...
  return (
    <SidebarProvider>
        ...
        <main className="flex-1 flex flex-col h-screen w-full overflow-hidden">
          <header className="bg-transparent px-8 py-6 flex justify-between items-center shrink-0 w-full z-10">
            <div className="flex items-center gap-8">
              ...
              <div className="relative w-64 hidden md:block">
                ...
                <Input 
                  ...
                  className="h-10 pl-9 rounded-full bg-slate-100/50 border-none text-xs focus-visible:ring-gold/30"
                  value={searchQuery}
            ...
            <div className="flex gap-3">
              <Button 
                ...
                variant="outline" 
                size="icon"
                className="rounded-full h-11 w-11 border-gold/10 text-gold hover:bg-gold/5"
              >
                <RefreshCcw size={16} className={isInitialLoading ? "animate-spin" : ""} />
              </Button>
              <Button onClick={() => setShowForm(!showForm)} variant={showForm ? "outline" : "gold"} className="gap-2 rounded-full font-bold uppercase tracking-widest text-[10px] h-11 px-7 shadow-sm transition-all">
          ...
          <div className="flex-1 flex overflow-hidden w-full px-6 pb-6 gap-6">
                ...
                <div className="p-8 h-full flex flex-col">
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <div className="space-y-2 pt-2">
                      <label className="text-[10px] uppercase font-bold tracking-widest text-gold ml-1 block">Portfolio Assignment</label>
                      <Popover open={openDropdown} onOpenChange={setOpenDropdown}>
                        ...
                        <PopoverContent side="bottom" align="start" className="w-[336px] p-0 rounded-2xl border-gold/10 shadow-2xl z-[100]">
                          <Command className="rounded-2xl">
                            <CommandInput placeholder="Search Schemes..." className="h-10" />
                  ...
                  <Button variant="gold" className="w-full h-14 rounded-2xl text-[11px] uppercase tracking-[0.2em] font-bold shadow-xl shadow-gold/20 mt-6 shrink-0" onClick={handleCreateCustomer} disabled={isCreating}>
                    {isCreating ? <Loader2 className="animate-spin w-4 h-4 mr-2" /> : null}
                    {isCreating ? "Processing..." : "Authorize Member"}
                  </Button>
                </div>
              </div>
            </aside>
            ...
            <section className="flex-1 flex flex-col min-w-0 bg-white border border-gold/10 rounded-[2.5rem] shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-8 py-6 border-b border-gold/5 bg-slate-50/30">
                ...
                <Badge variant="outline" className="rounded-full border-gold/20 text-gold bg-gold/5 px-4 py-1 text-[9px] uppercase font-bold tracking-widest">
                ...
                <div className={`grid gap-6 transition-all duration-500 ${showForm ? "grid-cols-1 xl:grid-cols-2" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"}`}>
                  {isInitialLoading ? Array.from({ length: 8 }).map((_, i) => <CustomerSkeleton key={i} />) : (
                    filteredCustomers.map((customer) => (
                      ...
                      >
                        <LuxuryCard className="p-6 rounded-[22px] shadow-sm hover:shadow-xl flex flex-col bg-white border-gold/5 border">
      ...
      {/* SUPERADMIN OVERRIDE DIALOG */}
      <Dialog open={!!selectedCustomer} onOpenChange={() => { setSelectedCustomer(null); setIsUpdatingPassword(false); }}>
        <DialogContent className="max-w-3xl rounded-[2.5rem] border-gold/20 bg-white/95 backdrop-blur-xl p-0 overflow-hidden shadow-2xl">
          <DialogHeader className="p-8 border-b border-gold/10 bg-slate-50/50">
            <div className="flex justify-between items-center w-full pr-10">
                ...
                <Button 
                  ...
                  className="rounded-full border-gold/20 text-gold text-[10px] font-bold uppercase px-4 h-9 gap-2 transition-all"
                >
                    {isUpdatingPassword ? <X size={14} /> : <KeyRound size={14} />} 
                    {isUpdatingPassword ? "Cancel" : "Reset Credentials"}
                </Button>
            </div>
          </DialogHeader>

          {isUpdatingPassword && (
            <div className="bg-gold/[0.03] border-b border-gold/10 px-8 py-6">
                <div className="flex items-end gap-4 max-w-md">
                    <div className="flex-1 space-y-2 text-left">
                        <label className="text-[10px] uppercase font-bold text-gold tracking-widest ml-1">Force New Password</label>
          ...
          <div className="p-8 max-h-[60vh] overflow-y-auto custom-scrollbar bg-white">
            <h4 className="text-[10px] uppercase font-bold tracking-[0.2em] text-gold mb-6 border-b border-gold/10 pb-2 flex items-center gap-2"><Layers size={12}/> Global Registry Profile</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-4">
              {selectedCustomer?.schemes.map((cs: any) => (
                <div key={cs.id} className="border border-gold/10 rounded-3xl p-6 bg-[#fafaf9] hover:bg-white transition-all text-left">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-5 font-bold text-[11px] text-slate-500">
                    ...
                    <div className="flex items-center gap-2"><TrendingUp size={13} className="text-gold/70"/> ₹{cs.scheme.maturityAmount}</div>
                  </div>

                  <div className="pt-4 border-t border-gold/5 flex justify-between items-end">
                    <div className="space-y-1">
                       <p className="text-[9px] uppercase font-bold text-muted-foreground tracking-tighter">Authorized Balance Rem.</p>
          ...
          <div className="p-6 bg-slate-50 border-t border-gold/10 flex justify-end">
            <Button variant="outline" onClick={() => { setSelectedCustomer(null); setIsUpdatingPassword(false); }} className="rounded-full px-10 font-bold text-[10px] uppercase tracking-widest border-gold/20 text-gold h-11 transition-all">Dismiss Profile</Button>
          </div>
        </DialogContent>
      </Dialog>
