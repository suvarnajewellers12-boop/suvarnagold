...
const Reports = () => {
  ...
  const exportToExcel = () => {
    try {
      const dataToExport = filteredData.map((p) => ({
        ...
        Customer: p.customer,
        Cash: p.payments.cash,
        UPI: p.payments.upi,
        Card: p.payments.card,
        Cheque: p.payments.cheque,
        Total: p.grandTotal,
  ...
  const handleReceiptAction = async (purchase: any, mode: "download" | "print") => {
    try {
      ...
      custY += 14;
      custY = drawWrapped(purchase.address || "", rightColX, custY, marginR - rightColX, 10, grey, 14);
      ...
      draw("ITEM DESCRIPTION", col.desc, headY, 10, grey);
      draw("PURITY", col.purity, headY, 10, grey);
      draw("GROSS WT", col.gross, headY, 10, grey);
      draw("NET WT", col.net, headY, 10, grey);
      draw("VA (%)", col.va, headY, 10, grey);
      drawRight("VALUE", col.value, headY, 10, grey);
      ...
      summaryY += 20;
      draw("Taxes (GST)", labelX, summaryY, 11);
      drawRight(`₹${(purchase.cgst + purchase.sgst).toLocaleString()}`, marginR, summaryY, 11);

      if (purchase.discount > 0) {
        summaryY += 20;
        draw("Discounts", labelX, summaryY, 11, red);
        drawRight(`- ₹${purchase.discount.toLocaleString()}`, marginR, summaryY, 11, red);
      }

      const totalY = summaryY + 30;
      draw("GRAND TOTAL", labelX, totalY, 13, gold);
      drawRight(`₹${purchase.grandTotal.toLocaleString()}`, marginR, totalY, 16, gold);
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      ...
                      <span className="text-xs flex items-center gap-1 opacity-80"><Calendar className="w-3 h-3"/> {selectedCustomer.date.toLocaleDateString()}</span>
                    </div>
                  </div>
